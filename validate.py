...
def val(args, accelerator, model, val_data, val_loader, loss_func, mae_loss_func, mse_loss_func):
    total_loss = []
    total_mae_loss = []
    total_mse_loss = []
    val_result = []
    ...
    if not os.path.exists('result'):
        os.makedirs('result')
    with open('result/validate_result.json', 'w') as f:
...
def main():
    ...
    if accelerator.is_main_process:
        print(f'valing Loss: {val_loss}, MAE Loss: {val_mae_loss}, MSE Loss: {val_mse_loss}')
